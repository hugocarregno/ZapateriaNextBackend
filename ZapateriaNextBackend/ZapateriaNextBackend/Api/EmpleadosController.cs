...
namespace ZapateriaNextBackend.Api
{
    ...
    public class EmpleadosController : ControllerBase
    {
        ...
        public async Task<IActionResult> Get(int id)
        {
            try
            {
                return Ok(contexto.Empleados.SingleOrDefault(x => x.Id == id));
            }
            catch (Exception ex)
            {
                return BadRequest(ex);
        ...
        public async Task<IActionResult> Put(int id, [FromBody] Empleado entidad)
        {
            try
            {
                ...
                if (ModelState.IsValid && contexto.Empleados.AsNoTracking().SingleOrDefault(e => e.Id == id) != null)
                {
                    //entidad.Id = id;
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                var entidad = contexto.Empleados.FirstOrDefault(e => e.Id == id && e.Email == User.Identity.Name);
                if (entidad != null)
                {
                    entidad.Estado = 0;
                    contexto.Empleados.Update(entidad);
